<template>
  <div class="audio-player">
    ...
    <audio :src="song.audioUrl" @pause="paused" @play="played" @timeupdate="updatePlayedTime" @canplay="prepared"
      @seeking="seeking" @seeked="seeked" autoplay loop ref="audio"></audio>
    ...
    <div class="play-controls">
      ...
      <ProgressBar :dotSize="8" :barHeight="4" :barColor="'green'" :barBgColor="'black'" :percent="percentage"
        :disabled="adjustDisabled" @change="seekAudio" />
    ...
    <div class="right-part">
      ...
      <ProgressBar :dotSize="10" :barHeight="4" :barColor="'#c33f18'" :barBgColor="'grey'" :percent="volumeRatio * 100"
        @change="adjustVolume" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updatePlayedTime() {
      ...
      if (this.duration > 0) {
        this.timeString =
          this.$helpers.convertSecToMinutes(this.currentTime) +
          " / " +
          this.$helpers.convertSecToMinutes(this.duration);
      } else {
  ...
  watch: {
    // change audio play status when video playing status changes
