...
export default {
	...
	methods: {
		loadCustomizedStyles() {
			...
			this.$refs.progressdot.style.top = "-" + (this.dotSize - this.barHeight) / 2 + "px";
			this.$refs.progressbar.style.borderRadius = this.barHeight / 2 + "px";
			this.$refs.progressline.style.borderRadiusLeft = this.barHeight / 2 + "px";
			this.$refs.progressline.style.borderBottomLeftRadius = this.barHeight / 2 + "px";

			this.$refs.progressline.style.borderTopLeftRadius = this.barHeight / 2 + "px";
		...
		mouseDown(event) {
			...
			this.curDotPosition.xOffest = event.touches ? event.touches[0].clientX : event.clientX;
			...
			if (this.curDotPosition.isDrag) {
				...
				if (xDistance > 0) {
					if (xDistance > xMaxValidRight) {
						// console.log("exceed the right border");
				...
				let newPercent = ((100 * this.curDotPosition.left) / maxValidDistance).toFixed(2);
				...
				// console.log(this.percent, this.curDotPosition.left, this.curDotPosition.xOffest, newPercent, maxValidDistance);
			}
		},
		...
		mouseClick(event) {
			...
			console.log("Clicked...", event.clientX, this.$refs.progressbar.getBoundingClientRect());
			let mouseClientX = event.clientX;

			// this left value is not the dot style.left value.
			const { x: dotClientX } = this.$refs.progressdot.getBoundingClientRect();
			// console.log("dotX dotLeft, clickX, totalWidth", dotClientX, dotLeft, mouseClientX, this.barTotalWidth);
			...
			let newPercent = Math.min(((100 * dotNewStyleLeft) / maxValidDistance).toFixed(2), 100);
		...
		screenResize() {
			try {
				let barTotalWidth = this.$refs.progressbar.getBoundingClientRect().width;
	...
	computed: {
		...
		updateBarPosition() {
			...
			let dotCurPosition = ((this.barTotalWidth - this.dotSize) * this.percentage) / 100;
